...
def application_approve(request, pk):
    ...
    # --- ensure registrar ---
    if not hasattr(request.user, "subregistrar_profile"):
...
def transactions_view(request):
    ...
    # adjust filter based on your model relation
    transactions = Transaction.objects.filter(
        customer__user=user
    ).select_related("customer").order_by("-submission_date")

    context = {
        "transactions": transactions
    }

    return render(request, "auth/transactions.html", context)


def transaction_detail(request, pk):
    request.session.set_expiry(300)
    request.session["last_transaction_id"] = pk
    ...
    REGISTRATION_FEE = Decimal("50.50")
    total_amount = tx.valuation + REGISTRATION_FEE
    ...
    return render(request, "auth/detail.html", context)

from django.urls import reverse
def auto_logout(request):
    """
    Auto logout after session expiry.
    Redirects to transaction page with expired flag.
    """
    tx_id = request.session.get("last_transaction_id")

    logout(request)
    request.session.flush()

    if tx_id:
        return redirect(f"{reverse('transaction_detail', kwargs={'pk': tx_id})}?expired=1")
...
def edit_customer_profile(request):
    ...
    if request.method == "POST":
        ...
        # --- Customer fields ---
        customer.phone_no = request.POST.get("phone_no", "").strip()
        customer.adhar_no = request.POST.get("adhar_no", "").strip()
        customer.pan_number = request.POST.get("pan_number", "").strip()
        customer.date_of_birth = parse_date(request.POST.get("date_of_birth"))
        ...
        customer.eth_address = request.POST.get("eth_address", "").strip()
